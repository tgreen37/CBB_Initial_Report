...
#knitr::opts_knit$set(root.dir = "D:/GCI/CBB R")
...
Create Matt's CSV file for Maps

```{r, matt csv}
#FIGURE 10 UNIQUE LOTS
# Filter out invalid ZIP codes
filtered_data <- data_clean %>%
  filter(!is.na(Property_ZipCode) & Property_ZipCode != 0)

# Get unique Property.PIN values
unique_lots <- filtered_data %>%
  distinct(Property.PIN, Property_ZipCode)

# Count unique lots by ZIP Code
zip_lot_counts <- unique_lots %>%
  group_by(Property_ZipCode) %>%
  summarise(Unique.Lots.by.ZIP = n(), .groups = "drop")

#FIGURE 10 UNIQUE INDIVIDUALS
# Filter dataset for Chicago applicants and valid ZIP codes
filtered_applicants <- data_clean %>%
  filter(Applicant.Address.City %in% c("Chicago", "Chicago, IL") & 
         !is.na(Applicant.ZIP.Code) & Applicant.ZIP.Code != 0)

# Keep only unique Name.Key per ZIP
unique_applicants <- filtered_applicants %>%
  distinct(Name.Key, Applicant.ZIP.Code)  # Ensures each applicant is counted only once per ZIP

# Count unique applicants by home ZIP
home_zip_counts <- unique_applicants %>%
  group_by(Applicant.ZIP.Code) %>%
  summarise(Applicant.Counts.by.Home.ZIP = n(), .groups = "drop")

#FIGURE 11 APPLICANTS FROM SAME ZIP APPLIED FOR
# Sum all application counts by Property_ZipCode
property_app_counts <- data_clean %>%
  filter(!is.na(Property_ZipCode) & Property_ZipCode != 0) %>%
  group_by(Property_ZipCode) %>%
  summarise(Property.Applications.by.ZIP = n(), .groups = "drop")

# Filter dataset where Property_ZipCode matches Applicant.ZIP
matching_zips <- data_clean %>%
  filter(Property_ZipCode == Applicant.ZIP.Code & 
         !is.na(Property_ZipCode) & Property_ZipCode != 0) %>%
  group_by(Property_ZipCode) %>%
  summarise(Property.Applicant.ZIP.Match.Count = n(), .groups = "drop")

# Merge the two datasets to calculate the percentage match
zip_summary <- property_app_counts %>%
  left_join(matching_zips, by = "Property_ZipCode") %>%
  mutate(Property.Applicant.ZIP.Match.Count = replace_na(Property.Applicant.ZIP.Match.Count, 0),  # Replace NA with 0
         Pct.Property.Applicant.ZIP.Match = Property.Applicant.ZIP.Match.Count / Property.Applications.by.ZIP * 100)

# Filter dataset for Sole Proprietor/Individual applicants
sole_proprietor_data <- data_clean %>%
  filter(Purchasing.Entity.Type == "Sole Proprietor/Individual")

# Sum all application counts by Property_ZipCode for Sole Proprietors
sole_proprietor_app_counts <- sole_proprietor_data %>%
  filter(!is.na(Property_ZipCode) & Property_ZipCode != 0) %>%
  group_by(Property_ZipCode) %>%
  summarise(Sole.Proprietor.Applications.by.ZIP = n(), .groups = "drop")

# Filter dataset where Property_ZipCode matches Applicant.ZIP for Sole Proprietors
sole_proprietor_matching_zips <- sole_proprietor_data %>%
  filter(Property_ZipCode == Applicant.ZIP.Code & 
         !is.na(Property_ZipCode) & Property_ZipCode != 0) %>%
  group_by(Property_ZipCode) %>%
  summarise(Sole.Proprietor.Applicant.ZIP.Match.Count = n(), .groups = "drop")

# Merge the two datasets to calculate the percentage match
sole_proprietor_summary <- sole_proprietor_app_counts %>%
  left_join(sole_proprietor_matching_zips, by = "Property_ZipCode") %>%
  mutate(Sole.Proprietor.Applicant.ZIP.Match.Count = replace_na(Sole.Proprietor.Applicant.ZIP.Match.Count, 0),  # Replace NA with 0
         Pct.Sole.Proprietor.Applicant.ZIP.Match = Sole.Proprietor.Applicant.ZIP.Match.Count / Sole.Proprietor.Applications.by.ZIP * 100)

# Filter dataset for non-Sole Proprietor (Organizations)
org_data <- data_clean %>%
  filter(Purchasing.Entity.Type != "Sole Proprietor/Individual")

# Sum all application counts by Property_ZipCode for Organizations
org_app_counts <- org_data %>%
  filter(!is.na(Property_ZipCode) & Property_ZipCode != 0) %>%
  group_by(Property_ZipCode) %>%
  summarise(Org.Applications.by.ZIP = n(), .groups = "drop")

# Filter dataset where Property_ZipCode matches Applicant.ZIP.Code for Organizations
org_matching_zips <- org_data %>%
  filter(Property_ZipCode == Applicant.ZIP.Code & 
         !is.na(Property_ZipCode) & Property_ZipCode != 0) %>%
  group_by(Property_ZipCode) %>%
  summarise(Org.Applicant.ZIP.Match.Count = n(), .groups = "drop")

# Merge the two datasets to calculate the percentage match
org_summary <- org_app_counts %>%
  left_join(org_matching_zips, by = "Property_ZipCode") %>%
  mutate(Org.Applicant.ZIP.Match.Count = replace_na(Org.Applicant.ZIP.Match.Count, 0),  # Replace NA with 0
         Pct.Org.Applicant.ZIP.Match = Org.Applicant.ZIP.Match.Count / Org.Applications.by.ZIP * 100)

# Count applications where Property_ZipCode does NOT match Applicant.ZIP.Code
outside_zip_applications <- data_clean %>%
  filter(Property_ZipCode != Applicant.ZIP.Code & 
         !is.na(Property_ZipCode) & Property_ZipCode != 0) %>%
  group_by(Property_ZipCode) %>%
  summarise(Outside.ZIP.Application.Count = n(), .groups = "drop")

# Start with a full join of all ZIP-related datasets
final_zip_summary <- zip_lot_counts %>%
  full_join(home_zip_counts, by = c("Property_ZipCode" = "Applicant.ZIP.Code")) %>%
  full_join(zip_summary, by = "Property_ZipCode") %>%
  full_join(sole_proprietor_summary, by = "Property_ZipCode") %>%
  full_join(org_summary, by = "Property_ZipCode") %>%
  full_join(outside_zip_applications, by = "Property_ZipCode")

# Replace NA values with 0 to avoid missing data
final_zip_summary[is.na(final_zip_summary)] <- 0
...
# Save to CSV
#write.csv(final_zip_summary, "figure.map.values.csv", row.names = FALSE)

rm(filtered_applicants, filtered_data, final_zip_summary, home_zip_counts, matching_zips, org_app_counts,
   org_data, org_matching_zips, org_summary, outside_zip_applications, property_app_counts,
   sole_proprietor_app_counts, sole_proprietor_data, sole_proprietor_matching_zips, 
   sole_proprietor_summary, unique_applicants, unique_lots, zip_lot_counts, zip_summary)
...
status_final_table %>%
  ...
  add_header_above(c(" " = 2, "Accepted" = 2, "In-Progress" = 2, "Declined & Withdrawn" = 2)) %>%
...
```
All Sole Proprietor/Individuals had demographic responses. "Not Provided" exploration no longer valid. 
...
Create CBB Demand score by Ward
Need unique lots by Ward
Total Volume of Applications by Ward
...
data_wards_cleaned$Application.Status2 <- data_wards_cleaned$Application.Status  # Step 1: Create the field
...
# Get unique Property.PIN values
unique_lots_commarea <- filtered_data %>%
  distinct(Property.PIN, Property.Community.Area.1)

# Count unique lots by Community Area
commarea_lot_counts <- unique_lots_commarea %>%
  group_by(Property.Community.Area.1) %>%
...
commarea_app_counts <- data_clean %>%
  filter(!is.na(Property_ZipCode) & Property_ZipCode != 0) %>%
  group_by(Property.Community.Area.1) %>%
...
commarea_summary <- commarea_lot_counts %>%
  full_join(commarea_app_counts, by = c("Property.Community.Area.1" = "Property.Community.Area.1")) 
