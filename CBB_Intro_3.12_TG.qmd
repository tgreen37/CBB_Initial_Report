...
```{r, warning=FALSE}
invisible(data_wards <- read_csv("data_prepped_2.18.csv", show_col_types = FALSE))

invisible(data_wards %>%
  filter(!str_detect(Property.PIN, "^[0-9]+$") | !str_detect(Property.Ward, "^[0-9]+$"))
)

invisible(data_wards_cleaned <- data_wards %>%
  mutate(
...
ward_status_pivot_table %>%
  ...
  kable(col.names = c("Ward", "Count of Applications", "Accepted (%)", "In-Progress (%)", "Declined & Withdrawn (%)"),
...
comm_area_status_pivot_table %>%
  kable(col.names = c("Community Area", "Count of Applications", "Applications/Lot", "Accepted (%)", "In-Progress (%)", "Declined & Withdrawn (%)"),
        caption = "Percent of Application Status by Community Area") %>%
  kable_styling(full_width = FALSE, 
                position = "left", 
                font_size = 12,  # Readable font size
                latex_options = "scale_down") %>%
  column_spec(1, width = "12em") %>%  # First column set to at least 25em
  column_spec(2:6, width = "12em") # Other columns set to 6em

```

```{r}

comm_area_analysis_table <- comm_area_status_pivot_table %>%
  mutate(Accepted.Applications = Total_Count * (Accepted / 100))

comm_area_analysis_table <- comm_area_analysis_table %>%
   mutate(Accepted.Applications = round(Accepted.Applications, 0))

ggplot(comm_area_analysis_table, aes(x = Commarea.Demand.Score, y = Accepted.Applications)) +
  geom_point(color = "blue", size = 3, alpha = 0.7) +  # Scatter plot points
  geom_smooth(method = "lm", se = TRUE, color = "red") +  # Line of best fit
  labs(title = "Scatter Plot of Community Area Demand Score vs Accepted Applications",
       x = "Demand Score",
       y = "Accepted Applications") +
  theme_minimal()

count_of_applications_per_lot
...
# Lots Applied For Count by Zip Code
...
50.32% of Applicant ZIPs match the Property ZIP

# ZIP Code Connections for resident applicants - Illinois
...
# change property_zips to sf object
property_points_sf <- st_as_sf(property_zips, coords = c("lon", "lat"), crs = 3435)

# Extract coordinates as separate columns
applicant_zips_points_sf <- applicant_zips_points_sf %>%
  mutate(lon_app = st_coordinates(.)[,1], lat_app = st_coordinates(.)[,2]) %>%
  st_drop_geometry()  # Drop spatial data for joining

property_points_sf <- property_points_sf %>%
  mutate(lon_prop = st_coordinates(.)[,1], lat_prop = st_coordinates(.)[,2]) %>%
  st_drop_geometry()  # Drop spatial data for joining

apptoprop_df <- inner_join(applicant_zips_points_sf, property_points_sf, by = "Record.ID")

line_geometries <- st_sfc(
  lapply(1:nrow(apptoprop_df), function(i) {
    st_linestring(rbind(
      c(apptoprop_df$lon_app[i], apptoprop_df$lat_app[i]),  # Start point
      c(apptoprop_df$lon_prop[i], apptoprop_df$lat_prop[i]) # End point
    ))
  }),
  crs = 4326  # Ensure correct CRS (change if needed)
)

# Convert the dataframe into an sf object with the geometry column
apptoprop_sf <- st_sf(apptoprop_df, geometry = line_geometries)

# Check to see if LINSTRING is return for all rows
st_geometry_type(apptoprop_sf)

# Ensure applicant points are sf objects
applicant_zips_points_sf <- st_as_sf(applicant_zips_points_sf, 
                                     coords = c("lon", "lat"), 
                                     crs = 3435)  # Adjust CRS as needed

# Ensure property points are sf objects
property_points_sf <- st_as_sf(property_points_sf, 
                               coords = c("lon_prop", "lat_prop"), 
                               crs = 3435)

ggplot() +
  geom_sf(data = apptoprop_sf, color = "black", linewidth = 1) +  # Lines connecting points
  geom_sf(data = st_as_sf(applicant_zips_points_sf), color = "blue", size = 3) +  # Start points
  geom_sf(data = st_as_sf(property_points_sf), color = "red", size = 3) +  # End points
  theme_minimal() +
  ggtitle("Lines Connecting Applicant and Property Points")

```

# ZIP Code Connections for resident applicants - Cook County
...
# change property_zips to sf object
# property_zips_points_sf <- st_as_sf(property_zips, coords = c("lon", "lat"), crs = 4269)
...
# Extract coordinates as separate columns
applicant_zips_points_sf <- applicant_zips_points_sf %>%
  mutate(INTPTLON10 = st_coordinates(.)[,1], INTPTLAT10 = st_coordinates(.)[,2]) %>%
  st_drop_geometry()  # Drop spatial data for joining
...
apptoprop_df <- inner_join(applicant_zips_points_sf, property_zips_points_sf, by = "Record.ID")


line_geometries <- st_sfc(
  lapply(1:nrow(apptoprop_df), function(i) {
    st_linestring(rbind(
      c(apptoprop_df$INTPTLON10.x[i], apptoprop_df$INTPTLAT10.x[i]),  # Start point
      c(apptoprop_df$INTPTLON10.y[i], apptoprop_df$INTPTLAT10.y[i]) # End point
    ))
  }),
...
#chi_map <- get_map(location = "Chicago, IL", zoom = 7)

 ggplot() +
    geom_sf(data = apptoprop_sf, color = "black", linewidth = 0.5) +  # Lines
    geom_sf(data = st_as_sf(applicant_zips_points_sf), color = "blue", size = 2) +  # Start points
    geom_sf(data = st_as_sf(property_zips_points_sf), color = "red", size = 2) +  # End points
    theme_minimal(base_size = 16) +  # Increase text size
    theme(plot.margin = margin(1, 1, 1, 1, "cm")) +  # Expand margins
    ggtitle("Lines Connecting Applicant Zips and Property Zips") +
    theme(plot.title = element_text(size = 18, face = "bold")) 

ggsave("Cook_County_ZCTA_Map.png", width = 12, height = 8, dpi = 300)
...
# ZIP Code Connections for Zip Code TA 60612
...
# Percent of applications from within the same ZIP code
...
tm_shape(percent_local_zips) +
  tm_polygons(fill = "percent_local", 
              fill.scale = tm_scale_intervals(n = 5, style = "jenks", values = "brewer.blues")) +
  tm_layout(legend.outside = TRUE, frame = FALSE) +
  tm_title("Percent of Applications from within ZIP Code")


```

# Percent of applications from within the same ZIP code for Individuals/Sole Proprietors only
...
# For zip codes that are the same for applicant and property, summarise applications (rows) by zip code and count the number of times that each zip appears in a new column called "local_count"
matched_zips <- data_clean_individual %>%
  filter(applicant_ZIP == Property_ZipCode) %>%
  group_by(applicant_ZIP) %>%
  summarise(local_count = n())

# For ALL zip codes where the applicant zip and property zip are the same, summarise applications (rows) by zip code and count the number of times that each zip appears in a new column called "total_count"
total_zips <- data_clean %>%
  filter(applicant_ZIP == Property_ZipCode) %>%
  group_by(applicant_ZIP) %>%
  summarise(total_count = n())
...
tm_shape(percent_local_zips) +
  ...
  tm_layout(legend.outside = TRUE, frame = FALSE) +
  tm_title("Percent of Applications from within ZIP Code for Sole Proprietors/Individuals")
...
# Property Applications by Ward

The following map demonstrates the quantity of application by city council ward.
The natural breaks grouping pattern isolates wards 16, 20, 24, and 28 as those with the highest number of applications, likely due to the high number of lots available in this wards.
Overall, applications are concentrated on the central part of the City's south side and on the west side, due west of the loop.

```{r, message=FALSE, warning=FALSE}
# Load wards shapefile and assign projection
wards <- read_sf("ward_boundaries.shp")
wards <- st_transform(wards, crs = 3435)

ward_boundaries_with_counts <- wards %>%
  left_join(ward_status_pivot_table, by = c("ward_id" = "Property.Ward"))

tm_shape(ward_boundaries_with_counts) +
  tm_polygons(col = "Total_Count", palette = "Blues", style = "jenks") +
  tm_layout(legend.outside = TRUE, frame = FALSE) +
  tm_title("Application Count by Ward")
```
